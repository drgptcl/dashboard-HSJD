...
def build_clean_df(raw: pd.DataFrame) -> pd.DataFrame:
    ...
    mapping = {
        ...
        "physician": find_col(cols, "physician name") or find_col(cols, "médico"),
...
st.caption(f"Última actualización: {datetime.now().strftime('%d-%m-%Y %H:%M:%S')} · {len(df)} pacientes registrados")
...
journey_steps = [
    ...
    ("Terapia Inducción", len(has_induction), f"Previo a cirugía"),
...
if not has_surgery.empty:
    # Agrupar por tipo de cirugía y contar complicaciones de forma limpia
    comp_analysis = []
    for sx in has_surgery["surgery_type"].unique():
        ...
        # FIX: Corrected matching curly brackets for the dictionary append
    ...
    st.info("No hay datos de cirugías registradas para realizar el análisis cruzado.")
...
with d2:
    ...
    # Consolidar volumen de fármacos o esquemas moleculares avanzados
    therapy_metrics = {
...
with e1:
    ...
    # Filtrar solo pacientes con exámenes pendientes anotados explicitamente
    ...
    if not exam_df.empty:
        ...
        st.info("No hay exámenes de control específicos programados para la cohorte actual.")

with e2:
    st.markdown("**Indicaciones Médicas Destacadas**")
    # Espacio para mostrar notas de control abiertas ingresadas por los médicos
    ...
    if medical_notes:
        ...
        st.info("No se registran observaciones médicas especiales en las últimas hojas.")
